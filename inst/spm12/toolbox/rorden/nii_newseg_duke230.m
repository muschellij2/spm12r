function nii_newseg_duke230 (P,norm, Template,T2);
%SPM8 new segment with the Duke-230 Tissue Probability Maps and tissue cleanup
...
for i=1:size(P,1)
  ...
  matlabbatch{1}.spm.tools.preproc8.channel.write = [0 1];% <- 1 bias correction used for rendering
  if nargin>3 %T2 specified
    ref2 = deblank(T2(i,:));
    [pth2,nam2,ext2] = spm_fileparts(ref2);
	fprintf('Using %s to segment T1 and T2 images: %s %s job %d/%d\n', Template, ref, ref2,i,size(P,1));
	...
	matlabbatch{1}.spm.tools.preproc8.channel(2).write = [0 0];
      	...
      	fprintf('Using %s to segment T1 %s job %d/%d\n', Template, ref,i,size(P,1));
  end;
