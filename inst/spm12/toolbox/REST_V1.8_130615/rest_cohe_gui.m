function varargout = rest_cohe_gui(varargin)
% GUI for Regional Homogeneity based on Coherence
%-----------------------------------------------------------
%	Copyright(c) 2007~2010
%	State Key Laboratory of Cognitive Neuroscience and Learning in Beijing Normal University
%	Based on SONG Xiao-Wei's ReHo GUI
%   For methodology, please see: Liu D, Yan C, Ren J, Yao L, Kiviniemi VJ and Zang Y (2010) Using coherence to measure regional homogeneity of resting-state fMRI signal. Front. Syst. Neurosci. 4:24. doi: 10.3389/fnsys.2010.00024 
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @rest_cohe_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @rest_cohe_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before rest_cohe_gui is made visible.
function rest_cohe_gui_OpeningFcn(hObject, eventdata, handles, varargin)    
	...
	handles.Cfg.OutputDir =pwd;			    % pwd is the default dir for Cohe map result
	handles.Cfg.WantMeanCoheMap ='Yes';		%Calcute the mean Cohe map default
	...
	movegui(handles.figCoheMain, 'center');
% 	set(handles.figCoheMain,'Name','Coherence Based on Region');
    ...
    % Choose default command line output for rest_cohe_gui
    handles.output = hObject;	 

    guidata(hObject, handles);% Update handles structure

    % UIWAIT makes rest_cohe_gui wait for user response (see UIRESUME)
    % uiwait(handles.figCoheMain);

% --- Outputs from this function are returned to the command line.
function varargout = rest_cohe_gui_OutputFcn(hObject, eventdata, handles)
...
function btnSelectDataDir_Callback(hObject, eventdata, handles)
    ...
    theDir =uigetdir(theDir, 'Please select the data directory to compute Cohe-ReHo map: ');
	if ischar(theDir),
		restGui_SetDataDir(hObject, theDir,handles);	
	end
...
function RecursiveAddDataDir(hObject, eventdata, handles)
	...
	theDir =uigetdir(theDir, 'Please select the parent data directory of many sub-folders containing EPI data to compute Cohe-ReHo map: ');
	if ischar(theDir),
		%Make the warning dlg off! 20071201
		setappdata(0, 'Cohe_DoingRecursiveDir', 1);
		theOldColor =get(handles.listDataDirs, 'BackgroundColor');
		set(handles.listDataDirs, 'BackgroundColor', [ 0.7373    0.9804    0.4784]);
		try
			rest_RecursiveDir(theDir, 'rest_cohe_gui(''restGui_SetDataDir'',gcbo, ''%s'', guidata(gcbo) )');
		catch
			rest_misc( 'DisplayLastException');
		end	
		set(handles.listDataDirs, 'BackgroundColor', theOldColor);
		rmappdata(0, 'Cohe_DoingRecursiveDir');
	end
...
function restGui_SetDataDir(hObject,ADir, handles)	
    ...
    % if (~isequal(ADir , 0)) &&( (size(handles.Cfg.DataDirs, 1)==0)||(0==seqmatch({ADir} ,handles.Cfg.DataDirs( : , 1) ) ) ) ,20070519
	if (~isequal(ADir , 0)) &&( (size(handles.Cfg.DataDirs, 1)==0)||(0==length(strmatch(ADir,handles.Cfg.DataDirs( : , 1),'exact' ) ) ))
		...
		if (theVolumnCount<=0),
			if isappdata(0, 'Cohe_DoingRecursiveDir') && getappdata(0, 'Cohe_DoingRecursiveDir'), 
			else
...
function UpdateDisplay(handles)
	...
	set(handles.ckboxDivideMean, 'Value', strcmpi(handles.Cfg.WantMeanCoheMap, 'Yes'));		
...
%% --- Executes on button press in btnComputeCohe.
function btnComputeCohe_Callback(hObject, eventdata, handles)
	if (size(handles.Cfg.DataDirs, 1)==0) %check legal parameter set first
		errordlg('No Data found! Please re-config'); 
		return;
	end
    if (exist('rest_Cohe_ReHo_Brain.m','file')==2)
		%write log 20070507
		...
		try
			...
			if size(handles.Cfg.DataDirs, 1)>1, 
				rest_waitbar(0,'Cohe-ReHo Batch Compution, wait ...','Cohe-ReHo','Parent');
            end
            pause(0.01);
            %add by Sandy Wang for parallel computing 20120404
            ...
            PF_edtLBC=str2num(get(handles.edtLBC,'String'));
            PF_edtHBC=str2num(get(handles.edtHBC,'String'));
            PF_Auto=handles.Cfg.Auto;
            PF_TimeP=str2num(get(handles.edtTimeP, 'String'));
            PF_Overlap=str2num(get(handles.edtOverlap, 'String'));
            
            PF_WantMeanCoheMap=handles.Cfg.WantMeanCoheMap;
            
            parfor x=1:size(handles.Cfg.DataDirs, 1)
				%Update display
		        ...
		        %LowB =str2num(get(handles.edtLBC, 'String'));
                %HighB =str2num(get(handles.edtHBC, 'String'));
		        %LowB =PF_edtLBC;
                %HighB =PF_edtHBC;
                
                %Auto=handles.Cfg.Auto;
                %TimeP=str2num(get(handles.edtTimeP, 'String'));
                %Overlap=str2num(get(handles.edtOverlap, 'String'));
                rest_Cohe_ReHo_Brain(PF_DataDirs{x}, ...
		                PF_ClusterCount, ...
		                PF_MaskFile, ...
                        theDstFile, ...
                        PF_SamplePeriod, ...								  
                        PF_edtHBC, ...
                        PF_edtLBC,PF_Auto,PF_TimeP,PF_Overlap);      % Revised by YAN Chao-Gan, 090420. Designate the input file path and output file path for the revised reho.m
				
				%20070504, divide ReHo brain by the mean within the mask
				%if strcmpi(handles.Cfg.WantMeanCoheMap, 'Yes')
                if strcmpi(PF_WantMeanCoheMap, 'Yes')
					theOrigCoheMap =theDstFile;				
					theMeanCoheMap =fullfile(theOutputDir,['m' thePrefix '_' ...
                                                GetDirName(PF_DataDirs{x}) ] );
												%GetDirName(handles.Cfg.DataDirs{x, 1}) ] );
				%	theMaskFile =handles.Cfg.MaskFile;
                    theMaskFile =PF_MaskFile;
					rest_DivideMeanWithinMask(theOrigCoheMap, theMeanCoheMap, theMaskFile);
				end
			end	
        ...
        errordlg('No rest_Cohe_ReHo_Brain.m ! Please re-install'); 
    end
...
function listDataDirs_Callback(hObject, eventdata, handles)
	...
	if strcmp(get(handles.figCoheMain, 'SelectionType'), 'open') %when double click 
...
function listDataDirs_KeyPressFcn(hObject, eventdata, handles)
    ...
    key =get(handles.figCoheMain, 'currentkey');
...
function ckboxDivideMean_Callback(hObject, eventdata, handles)
	if get(hObject,'Value')
		handles.Cfg.WantMeanCoheMap ='Yes';
	else	
		handles.Cfg.WantMeanCoheMap ='No';
	end
	guidata(hObject, handles);
	UpdateDisplay(handles);
...
function btnDivideMean_Callback(hObject, eventdata, handles)
	...
	try
	    ...
	    if (filename~=0)% not canceled
			...
			theOrigCoheMap =[pathname filename];
			theMeanCoheMap =[pathname 'm' filename];
			theMaskFile =handles.Cfg.MaskFile;
			rest_DivideMeanWithinMask(theOrigCoheMap, theMeanCoheMap, theMaskFile);
			msgbox(sprintf('Cohe-ReHo brain "%s.{hdr/img}" \ndivide its mean within mask successfully.\t\n\nSave to "%s.{hdr/img}"\n' , ... 
					theOrigCoheMap, theMeanCoheMap), ...				
...
function InitFrames(hObject,handles);
	offsetY =130; %dawnsong, 20070504, add for the divide by the mask mean, the Y of Edit "OutPut Diectory"
	% for Matlab 6.5 compatible, draw a panel
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+151 433 1]);
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+241 433 1]);
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[90 offsetY+152 1 90]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+152 1 233]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[435 offsetY+152 1 233]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+385 433 1]);	
	uicontrol(handles.figCoheMain,'Style','Text','Position',[152 offsetY+380 140 14],...
		'String','Input Parameters');
	uicontrol(handles.figCoheMain,'Style','Text','Position',[8 offsetY+238 40 14],...
		'String','Cluster');	
	uicontrol(handles.figCoheMain,'Style','Text','Position',[208 offsetY+238 40 14],...
		'String','Mask');	
    
	offsetY=offsetY-50;
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+94 433 1]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY-8 433 1]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY-8 1 102]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[435 offsetY-8 1 102]);	
	uicontrol(handles.figCoheMain,'Style','Text','Position',[152 offsetY+88 180 14],...
		'String','Output Parameters (Cohe-ReHo map)');
    
	offsetY=offsetY+50;	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+400 433 1]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[435 offsetY+400 1 50]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+400 1 50]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+450 433 1]);	
	uicontrol(handles.figCoheMain,'Style','Text','Position',[142 offsetY+445 180 14],...
		'String','Option: Ideal Band Pass Filter');
	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+465 433 1]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[435 offsetY+465 1 50]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+465 1 50]);	
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY+515 433 1]);	
	uicontrol(handles.figCoheMain,'Style','Text','Position',[142 offsetY+505 180 14],...
		'String','Option: Remove Linear Trend');
	
	offsetY=offsetY-50;
	%20070506, Add manual operation button groups like SPM
	uicontrol(handles.figCoheMain, 'Style','Frame','Position',[2 offsetY-25 433 1]);	
	uicontrol(handles.figCoheMain,'Style','Text','Position',[152 offsetY-30 140 14],...
...
function InitControlProperties(hObject, handles)
	...
	% --- FIGURE -------------------------------------
    offset=50;
	set(handles.figCoheMain,...
		'Units', 'pixels', ...
		'Position', [10 5 440 600+offset], ...
		'Name', 'Regional Homogeneity based on Coherence', ...
		'MenuBar', 'none', ...
		'NumberTitle', 'off', ...		
	...
	set(handles.text2,	...
		'Style', 'text', ...
		'Units', 'pixels', ...
		'Position', [16 186+offset 175 40], ...
		'FontSize', 24, ...
		'FontWeight', 'bold', ...
        'ToolTipString','Regional Homogeneity based on Coherence', ...
		'String', 'Cohe-ReHo');
	...
	set(handles.btnSelectMask,	...
		'Style', 'pushbutton', ...
		'Units', 'pixels', ...
		'Position', [396 240+offset 30 25], ...
		'FontSize', 18, ...
		'String', '...', ...
		'Enable', 'off', ...
		'CData', zeros(1,0));

	set(handles.btnComputeCohe,	...
		'Style', 'pushbutton', ...
		'Units', 'pixels', ...
		'Position', [319 238 105 30], ...
		'FontSize', 12, ...
		'FontWeight', 'bold', ...
		'String', 'Do all');
	...
	set(handles.rbtn7voxels,	...
		'Style', 'radiobutton', ...
		'Units', 'pixels', ...
		'Position', [10 298+offset 70 16], ...
		'String', '7 voxels');

	set(handles.rbtn19voxels,	...
		'Style', 'radiobutton', ...
		'Units', 'pixels', ...
		'Position', [10 273+offset 70 16], ...
		'String', '19 voxels');

	set(handles.rbtn27voxels,	...
		'Style', 'radiobutton', ...
		'Units', 'pixels', ...
		'Position', [10 248+offset 70 16], ...
		'String', '27 voxels');

	set(handles.rbtnDefaultMask,	...
		'Style', 'radiobutton', ...
		'Units', 'pixels', ...
		'Position', [110 297+offset 158 16], ...
		'String', 'Default mask');

	set(handles.rbtnUserMask,	...
		'Style', 'radiobutton', ...
		'Units', 'pixels', ...
		'Position', [110 271+offset 148 16], ...
		'String', 'User-defined mask');

	set(handles.rbtnNullMask,	...
		'Style', 'radiobutton', ...
		'Units', 'pixels', ...
		'Position', [277 298+offset 82 16], ...
		'String', 'No mask');
	...
	set(handles.ckboxFilter, ...
		'Style', 'checkbox', ...
		'Units', 'pixels', ...
		'Position', [14 491+offset 80 22], ...
		'String', 'Band (Hz)');
	set(handles.ckboxRetrend, ...		
		'Style', 'checkbox', ...
		'Units', 'pixels', ...
		'Visible', 'off', ...
		'Position', [366 490+offset 60 22], ...
		'String', 'Retrend');
	...
	uimenu(handles.hContextMenu, 'Label', 'Remove selected directory', 'Callback', 'rest_cohe_gui(''DeleteSelectedDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', 'Add recursively all sub-folders of a directory', 'Callback', 'rest_cohe_gui(''RecursiveAddDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', '=============================');	
	uimenu(handles.hContextMenu, 'Label', 'Remove all data directories', 'Callback', 'rest_cohe_gui(''ClearDataDirectories'',gcbo,[], guidata(gcbo))');
...
function Log2File(handles)
	...
	theMsgHead = sprintf('Cohe-ReHo computation log %s\r\n%s\r\n', rest_misc( 'GetDateTimeStr'), constLineSep);
	...
	theMsg =sprintf('%s\r\nCohe input parameters:\r\n%s\r\n\r\n%s', theMsg, ...
					LogCoheInputParameters(handles), constLineSep);
	theMsg =sprintf('%s\r\nCohe output parameters:\r\n%s\r\n\r\n%s', theMsg, ...
					LogCoheOutputParameters(handles), constLineSep);
...
function LogPerformance(handles)	
	theMsg =sprintf('\r\n\r\nTotal elapsed time for Regional Coherence Computing: %g  seconds\r\n',handles.Performance);
...
function ResultLogString=LogCoheInputParameters(handles)
	ResultLogString ='';
	constLineSep= '-------------------------------------------------------------------------------';
	theDataDirString= '';
...
function ResultLogString=LogCoheOutputParameters(handles)
	ResultLogString ='';
	...
	ResultLogString =sprintf('%s\tWant mean Cohe-ReHo map computation: %s \r\n',ResultLogString, handles.Cfg.WantMeanCoheMap);
...
function ResultLogFileName=GetLogFilename(ALogDirectory, APrefix)
	...
	ResultLogFileName=sprintf('%s%s_%sCohe.log', ...
		ALogDirectory, ...
		APrefix, ...
...
function edtLBC_Callback(hObject, eventdata, handles)

function edtLBC_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
function edtHBC_Callback(hObject, eventdata, handles)
function edtHBC_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end


function rtnAuto_Callback(hObject, eventdata, handles)
handles.Cfg.Auto =1;
guidata(hObject, handles);
set(handles.rtnManual,'Value',0);
set(handles.rtnAuto,'Value',1);
set(handles.edtTimeP,'Visible','off');
set(handles.txtTimeP,'Visible','off');
set(handles.edtOverlap,'Visible','off');
set(handles.txtOverlap,'Visible','off');

% --- Executes on button press in rtnManual.
function rtnManual_Callback(hObject, eventdata, handles)
handles.Cfg.Auto =0;
guidata(hObject, handles);
set(handles.rtnManual,'Value',1);
set(handles.rtnAuto,'Value',0);
set(handles.edtTimeP,'Visible','on');
set(handles.txtTimeP,'Visible','on');
set(handles.edtOverlap,'Visible','on');
set(handles.txtOverlap,'Visible','on');



function edtTimeP_Callback(hObject, eventdata, handles)

function edtTimeP_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
function edtOverlap_Callback(hObject, eventdata, handles)
function edtOverlap_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
