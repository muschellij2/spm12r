...
% Last revised by YAN Chao-Gan, 101101. Fixed a bug in processing multiple subjects with different covaribles in batch mode.
...
%              %%YAN Chao-Gan, 101101. Fixed a bug in processing multiple subjects with different covaribles in batch mode.
               Subject_Covariables.ort_file=CovariablesList{x+1};  
         end
   end
   rest_RegressOutCovariates(handles.Cfg.DataDirs{x, 1},Subject_Covariables,strcat('_',thePrefix),handles.Cfg.MaskFile);
   fprintf('\n\t Done');
  cd(theOldDir);
