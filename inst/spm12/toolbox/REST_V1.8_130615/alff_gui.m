...
function Result=GetInputDirDisplayList(handles)
	...
	for x=1:size(handles.Cfg.DataDirs,1)
        ...
        %Change by Sandy for select order of Dir 201207
        Result =[Result;...
            {sprintf('%d# %s',handles.Cfg.DataDirs{x, 2},handles.Cfg.DataDirs{x, 1})}];
	end
...
function btnComputeAlff_Callback(hObject, eventdata, handles)
    ...
    if (exist('alff.m','file')==2)
		...
		set(handles.listDataDirs, 'Value', 1);%Added by Sandy
        drawnow;
		try
            ...
            %Add by Sandy Wang 20120326 for parallel computing
            ...
            PF_ALFF_SamplePeriod=handles.ALFF.SamplePeriod;
            PF_ALFF_BandHigh=handles.ALFF.BandHigh;
            PF_ALFF_BandLow=handles.ALFF.BandLow;
            PF_MaskFile=handles.Cfg.MaskFile;
            PF_WantMeanAlffMap=handles.Cfg.WantMeanAlffMap;
            %for x=1:size(handles.Cfg.DataDirs, 1)
            tic;
            parfor x=1:size(PF_DataDirs,1)
				...
				theDstFile=fullfile(theOutputDir,[thePrefix,'_',GetDirName(PF_DataDirs{x})]);
                                            ...
                                            %add by Sandy Wang 20120326
		        alff(   PF_DataDirs{x},...%handles.Cfg.DataDirs{x, 1}, ... add by Sandy Wang 20120326
						PF_ALFF_SamplePeriod,...%handles.ALFF.SamplePeriod, ...	
						PF_ALFF_BandHigh,...%handles.ALFF.BandHigh, ...
						PF_ALFF_BandLow,...%handles.ALFF.BandLow, ...		                
		                PF_MaskFile,...%handles.Cfg.MaskFile, ...
                ...
                if strcmpi(PF_WantMeanAlffMap, 'Yes')
                    ...
                    %GetDirName(handles.Cfg.DataDirs{x, 1}) ] ); 20120326
...
function InitControlProperties(hObject, handles)
	...
	uimenu(handles.hContextMenu, 'Label', 'Remove all data directories', 'Callback', 'alff_gui(''ClearDataDirectories'',gcbo,[], guidata(gcbo))');
...
%Revised by YAN Chao-Gan, 100420. Fixed the bug in calculating the frequency band.
	...
	BandLowIdx		=handles.ALFF.BandLow * paddedLength * handles.ALFF.SamplePeriod + 1; %Revised by YAN Chao-Gan, 100420. Fixed the bug in calculating the frequency band. %BandLowIdx		=handles.ALFF.BandLow * paddedLength * handles.ALFF.SamplePeriod;
	rBandLowIdx		=round(BandLowIdx);
	rBandLow		=(rBandLowIdx - 1) /paddedLength /handles.ALFF.SamplePeriod; %Revised by YAN Chao-Gan, 100420. Fixed the bug in calculating the frequency band. %rBandLow		=rBandLowIdx /paddedLength /handles.ALFF.SamplePeriod;
	BandHighIdx		=handles.ALFF.BandHigh * paddedLength * handles.ALFF.SamplePeriod + 1; %Revised by YAN Chao-Gan, 100420. Fixed the bug in calculating the frequency band. %BandHighIdx		=handles.ALFF.BandHigh * paddedLength * handles.ALFF.SamplePeriod;
	rBandHighIdx	=round(BandHighIdx);
	rBandHigh		=(rBandHighIdx - 1) /paddedLength /handles.ALFF.SamplePeriod; %Revised by YAN Chao-Gan, 100420. Fixed the bug in calculating the frequency band. %rBandHigh		=rBandHighIdx /paddedLength /handles.ALFF.SamplePeriod;
