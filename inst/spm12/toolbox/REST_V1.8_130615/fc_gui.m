...
% Revised by YAN Chao-Gan 100130, fixed the bug in ROI-wise functional connectivity calculation.
% Revised by YAN Chao-Gan, 101025. Will not remove linear trend in regressing out covariables step.
% Revised by Sandy Wang, 120805, change the color of button (ROI-wise or Voxel-wise) which is selected by User.
% Revised by Sandy Wang, 120805, change the order when user load a subject's dataset, now rest will load subjects' dataset from 1 to ... 
...
function UpdateDisplay(handles)
	...
	if isempty(handles.Cfg.ROIList) || prod(size(handles.Cfg.ROIList)) ==0,
		...
		set(handles.btnROIVoxelWise, 'BackgroundColor','default', 'ForegroundColor', 'red', 'FontWeight', 'normal');
		set(handles.btnROIRegionWise, 'BackgroundColor','default', 'ForegroundColor', 'red', 'FontWeight', 'normal');		
	else
		% [pathstr, name, ext] = fileparts(handles.Cfg.ROIFile);
		% if strcmpi(ext, 'rest_roi'),		
		%if  ~iscell(handles.Cfg.ROIList) || 0 ...
        if strcmp(VoxelorROI_tag,'Voxel')
			% ( size(handles.Cfg.ROIList,1)==1 && ~isspace(handles.Cfg.ROIList) ),
			%Single Region for Voxel Wise type ROI			
			set(handles.btnROIVoxelWise,'BackgroundColor','blue','ForegroundColor', 'yellow', 'FontWeight', 'bold');
			set(handles.btnROIRegionWise,'BackgroundColor','default','ForegroundColor', 'red', 'FontWeight', 'normal');		
			
		%elseif size(handles.Cfg.ROIList,1)>=1,
        elseif strcmp(VoxelorROI_tag,'ROI')
			%Region Wise type ROI series
			set(handles.btnROIVoxelWise, 'BackgroundColor', 'default','ForegroundColor', 'red', 'FontWeight', 'normal');
			set(handles.btnROIRegionWise, 'BackgroundColor','blue','ForegroundColor', 'yellow', 'FontWeight', 'bold');		
		end
	end
...
function btnROIVoxelWise_Callback(hObject, eventdata, handles)
	...
	%	rest_SetROI('Delete');
	%end
    %Altered by Sandy Wang for computing multi-ROI FC image
    ...
    %Add a tag to choose Voxel-wise or ROI-wise
    %global VoxelorROI_tag;
    global VoxelorROI_tag;
    VoxelorROI_tag='Voxel';
	guidata(hObject, handles);
	UpdateDisplay(handles);
    %theDefaultColor=get(hObject,'BackgroundColor');
    %set(hObject,'BackgroundColor', 'blue','ForegroundColor','yellow','FontWeight','Bold');
    %set(handles.btnROIRegionWise,'BackgroundColor','default');
...
function btnROIRegionWise_Callback(hObject, eventdata, handles)
    ...
    %theDefaultColor=get(hObject,'BackgroundColor');
    %set(hObject,'BackgroundColor', 'blue','ForegroundColor','yellow','FontWeight','Bold');
    %set(handles.btnROIVoxelWise,'BackgroundColor','default');
...
function btnComputeFC_Callback(hObject, eventdata, handles)
	...
	if iscell(handles.Cfg.ROIList) && size(handles.Cfg.ROIList,1)<2 && strcmp(VoxelorROI_tag,'ROI')
    ...
    if (exist('fc.m','file')==2)
		...
		try
            ...
            %Add by Sandy Wang 20120423 for parallel computing
            PF_edtOutputDir=get(handles.edtOutputDir, 'String');
            PF_edtPrefix =get(handles.edtPrefix, 'String');
            PF_DataDirs=handles.Cfg.DataDirs(:,1);
            PF_MaskFile=handles.Cfg.MaskFile;
            PF_Subject_Covariables = handles.Covariables;
            PF_ort_file=handles.Covariables.ort_file;
            PF_ROIList=handles.Cfg.ROIList;
            PF_WantFisherZMap=handles.Cfg.WantFisherZMap;
            PF_VoxelorROI_tag=VoxelorROI_tag;
			%for x=1:size(handles.Cfg.DataDirs, 1)
            tic
            parfor x=1:size(PF_DataDirs, 1)
				%Update display
				...
				%Add by Sandy Wang 20120326
                %thePrefix =get(handles.edtPrefix, 'String');
                theOutputDir=PF_edtOutputDir;
				thePrefix=PF_edtPrefix;
				theDstFile=fullfile(theOutputDir,[thePrefix,'_',GetDirName(PF_DataDirs{x})]);
                ...
                Subject_Covariables = PF_Subject_Covariables;
                if ~isempty(PF_ort_file)  % Revised by YAN Chao-Gan 080903, in order to fix the bug that with no Covariables
                    CovariablesList=textread(PF_ort_file,'%s');
                ...
                if ~iscell(PF_ROIList)    
                    %[pathstr, name, ext] = fileparts(handles.Cfg.ROIList) 
                    [pathstr, name, ext] = fileparts(PF_ROIList);
                    if strcmpi(ext,'.txt')
                        %SeedTimeCourseList=textread(handles.Cfg.ROIList,'%s')
                        SeedTimeCourseList=textread(PF_ROIList,'%s');
                ...
                if strcmpi(PF_WantFisherZMap, 'Yes')
					if iscell(ResultMaps) && size(ResultMaps, 1)>1 ,
						%There are many correlation maps
						for y=1:size(ResultMaps, 1),
							theOrigFCMap =ResultMaps{y};
							[pathstr, name, ext] = fileparts(theOrigFCMap);
							theFisherZMap =[pathstr, filesep, 'z', name, ext];
							%theMaskFile =handles.Cfg.MaskFile;
                            theMaskFile =PF_MaskFile;
							rest_Corr2FisherZ(theOrigFCMap, theFisherZMap, theMaskFile);
						end
					elseif iscell(ResultMaps) && size(ResultMaps, 1)==1,
...
function FishZ_ROI_wise(ResultMaps,theFisherZMap)
    ResultMaps =0.5 * log((1 +ResultMaps)./(1- ResultMaps));
    save([theFisherZMap, '.txt'], 'ResultMaps', '-ASCII', '-DOUBLE','-TABS');
