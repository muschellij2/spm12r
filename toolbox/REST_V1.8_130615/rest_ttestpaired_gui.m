function varargout = rest_ttestpaired_gui(varargin)
%   varargout = rest_ttestpaired_gui(varargin)
%   Paired T-Test GUI.
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @rest_ttestpaired_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @rest_ttestpaired_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
function rest_ttestpaired_gui_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = rest_ttestpaired_gui_OutputFcn(hObject, eventdata, handles)
...
if lenC(1) >=2
    msgbox('Only two conditions are allowed in Paired T-Test.','Warning');
else
...
uimenu(handles.hContextMenu, 'Label', 'Remove selected group', 'Callback', 'rest_ttestpaired_gui(''DeleteSelectedG'',gcbo,[], guidata(gcbo))');
uimenu(handles.hContextMenu, 'Label', '=============================');
uimenu(handles.hContextMenu, 'Label', 'Remove all data groups', 'Callback', 'rest_ttestpaired_gui(''ClearG'',gcbo,[], guidata(gcbo))');
...
for x=size(handles.Cfg.GroupDirs, 1):-1:1
    if x ==1
        Result =[{sprintf('Condition 1: %d img %s',handles.Cfg.GroupDirs{x, 2},handles.Cfg.GroupDirs{x, 1})} ;Result];
    else
        Result =[{sprintf('Condition 2: %d img %s',handles.Cfg.GroupDirs{x, 2},handles.Cfg.GroupDirs{x, 1})} ;Result];
    end
end
