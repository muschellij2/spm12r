...
try    
    ...
    for i=1:size(PF_inputdata ,1) 
        ...
        if i==1
            PF_list={[PF_output , filesep ,output_prefix, input_name]};
            if fisherz_tag
                mPF_list={[PF_output , filesep , 'z' ,output_prefix, input_name]};
            end
        else
            PF_list=[PF_list  ; {[PF_output , filesep , output_prefix,input_name]}];
            if fisherz_tag            
                mPF_list=[mPF_list  ; {[PF_output , filesep , 'z' , output_prefix,input_name]}];
            end
        end
    end
    
    parfor i=1:size(PF_inputdata ,1)
        [VMHCBrain, Header]=rest_VMHC(  PF_inputdata{i}, ...
                                PF_list{i},       ...
                                mask_file,       ...
                                NeedDetrend,     ...
                                NeedBandpass,    ...
