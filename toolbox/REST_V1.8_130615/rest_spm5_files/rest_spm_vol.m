...
[pth,nam,ext,n1] = rest_spm_fileparts(deblank(p));
...
if isempty(n),
    V = rest_spm_vol_nifti(p);
else
    V = rest_spm_vol_nifti(p,n);
end;
    
if isempty(n) && length(V.private.dat.dim) > 3
    V0(1) = V;
    for i = 2:V.private.dat.dim(4)
        V0(i) = rest_spm_vol_nifti(p, i);
    end
    V = V0;
end
...
str = {...
	...
	['        ',rest_spm_str_manip(q,'k40d')],...
	' ',...
	'Please check that it is in the correct format.'};
rest_spm('alert*',str,mfilename,sqrt(-1));
return;
%_______________________________________________________________________
