function [t,sts] = rest_spm_select(varargin)
% File selector
    ...
    switch lower(varargin{1})
        ...
        case {'list', 'fplist', 'extlist', 'extfplist'}
            ...
            if regexpi(varargin{1}, 'fplist') % return full pathnames
                direc = rest_spm_select('cpath', varargin{2});
...
if rest_spm_matlab_version_chk('7') >=0
...
if ( rest_spm_matlab_version_chk('7') >= 0 ) && isdeployed,
    ...
    for j=1:numel(varargin),
        ...
        if ischar(varargin{j}),
            for i=1:size(varargin{j},1),
                [pth{i} n e v] = rest_spm_fileparts(deblank(varargin{j}(i,:)));
                fle{i} = [n e v];
            end;
...
if isempty(pth),
    for k = 1:numel(fle)
    [unused,nam,ext,num] = rest_spm_fileparts(fle{k});
    if ~isempty(num),
        ind = [str2num(num) 1 1];
        ind = ind(1);
    else
...
switch rest_spm_platform('filesys'),
case 'unx',
    mch = '^/';
    fs  = '/';
    fs1 = '/';
