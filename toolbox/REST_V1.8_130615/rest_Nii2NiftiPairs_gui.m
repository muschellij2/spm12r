function varargout = rest_Nii2NiftiPairs_gui(varargin)
%   varargout = rest_Nii2NiftiPairs_gui(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @rest_Nii2NiftiPairs_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @rest_Nii2NiftiPairs_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before rest_Nii2NiftiPairs_gui is made visible.
function rest_Nii2NiftiPairs_gui_OpeningFcn(hObject, eventdata, handles, varargin)
...
movegui(handles.rest_Nii2NiftiPairs_gui, 'center');
guidata(hObject, handles);

% --- Outputs from this function are returned to the command line.
function varargout = rest_Nii2NiftiPairs_gui_OutputFcn(hObject, eventdata, handles)
...
for i=1:size(handles.Cfg.DataDirs, 1)	
	...
	for x = 1:size(struct2cell(theFileList),2)
	    if strcmpi(theFileList(x).name(end-3:end), '.nii') 
				ImgFileList=[ImgFileList; {[handles.Cfg.DataDirs{i},filesep,theFileList(x).name(1:end-4),'.nii']}];
	    end
    end
end
...
function InitControlProperties(hObject, handles)
	...
	uimenu(handles.hContextMenu, 'Label', 'Remove selected directory', 'Callback', 'rest_Nii2NiftiPairs_gui(''DeleteSelectedDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', 'Add recursively all sub-folders of a directory', 'Callback', 'rest_Nii2NiftiPairs_gui(''RecursiveAddDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', '=============================');	
	uimenu(handles.hContextMenu, 'Label', 'Remove all data directories', 'Callback', 'rest_Nii2NiftiPairs_gui(''ClearDataDirectories'',gcbo,[], guidata(gcbo))');
...
function [nVolumn]=CheckDataDir(ADataDir)
	...
	theImgFiles=dir(fullfile(ADataDir,'*.nii'));
    count = 3; nVolumn = 0;		
    for count = 3:size(struct2cell(theFilenames),2)
        if	(length(theFilenames(count).name)>4) && ...
                strcmpi(theFilenames(count).name(end-3:end) , '.nii')
            nVolumn = nVolumn + 1;
        end
    end
...
function RecursiveAddDataDir(hObject, eventdata, handles)
	...
	if ischar(theDir),
		...
		try
			rest_RecursiveDir(theDir, 'rest_Nii2NiftiPairs_gui(''SetDataDir'',gcbo, ''%s'', guidata(gcbo) )');
		catch
