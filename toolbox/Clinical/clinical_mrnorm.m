function clinical_mrnorm(anatomical, lesion, pathological, vox, bb, DeleteIntermediateImages, UseTemplateMask, Modality);
...
fprintf('MR normalization version 3/3/2012 - for use with low-resolution or low-contrast images that do not allow accurate normalization-segmentation\n');
 ...
 if (exist(ref ) ~= 2) 
   ...
   elseif Modality== 3
	FLAIRtemplate  = fullfile(fileparts(which(mfilename)),'gg-flair-181-asym-8mmFWHM.nii');
   	fprintf('clinical_mrnorm: using FLAIR template from David Glahn''s team %s\n', FLAIRtemplate );
